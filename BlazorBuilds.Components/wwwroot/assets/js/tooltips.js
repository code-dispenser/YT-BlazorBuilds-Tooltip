...
*/
const _tooltipHandlers = new WeakMap();
const _closeButtonHandlers = new WeakMap();
/*
    * Set to store open tooltips, should just be one.
*/
const _openTooltips = new Set(); //store 
/*
    * Arrow function just to add the hide tooltip modifier class
 */
const addHideTooltipClass = (tooltip, hideTooltipClass) => tooltip.classList.add(hideTooltipClass);
/*
...
*/
function checkInitialTooltipPosition(tooltip, configuration) {
    ...
    const tooltipSpan = tooltip.querySelector(searchClass);
    if (!tooltipSpan)
        return;
    /*
...
function checkAndMoveTooltipPosition(tooltipSpan, divRect, spanRect, modifierClassName, configuration) {
    /*
        * Check if the current modifier class is still ok if not check the next one.
    */
    if (true === ModifierWithinBounds(configuration.tooltipTextClass, divRect, spanRect, modifierClassName, configuration.tooltipGapSize))
        return;
    const startIndex = configuration.tooltipModifiers.indexOf(modifierClassName);
    const classCount = configuration.tooltipModifiers.length;
...
function ModifierWithinBounds(tooltipClassName, divRect, spanRect, modifierClassName, tooltipGapSize) {
...
*/
function checkSetAriaAttribute(childElement, controlID, ariaAttribute) {
    if (childElement && false === childElement.hasAttribute(ariaAttribute))
        childElement.setAttribute(ariaAttribute, controlID);
}
function handleKeyDownEvent(keyDownEvent, configuration, focusedItem = false) {
    if (_openTooltips.size === 0)
        return;
    ...
    if (keyDownEvent.key.toLowerCase() === configuration.closeTooltipKey.toLowerCase()) {
        _openTooltips.forEach(tooltip => addHideTooltipClass(tooltip, configuration.hideTooltipClass));
        if (focusedItem)
            keyDownEvent.stopPropagation(); //be restrictive when its the wrapped input handler and not the document event handler.
    }
}
/*
...
*/
const createMouseEnterHandler = (tooltip, configuration) => () => checkInitialTooltipPosition(tooltip, configuration);
const createFocusEnterHandler = (tooltip, configuration) => () => checkInitialTooltipPosition(tooltip, configuration);
const createCloseHandler = (tooltip, hideTooltipClass) => () => addHideTooltipClass(tooltip, hideTooltipClass);
const createKeydownHandler = (configuration, focusedItem) => ((event) => handleKeyDownEvent(event, configuration, focusedItem));
function configurTooltipTrigger(tooltip, configuration) {
    const focusableElements = tooltip.querySelectorAll(_focusElements);
    if (focusableElements.length > 0) {
        const focusableChild = focusableElements[0];
...
function configureGlobalCloseHandler(configuration) {
    if (false === _isEscapeKeyHandlerRegistered)
        document.addEventListener("keydown", (event) => handleKeyDownEvent(event, configuration));
    /*
...
function configureTooltipCloseButton(tooltip, configuration, map) {
    ...
    const closeSpanButton = tooltip.querySelector(searchClass);
    if (!closeSpanButton)
        return;
...
function registerTooltip(tooltip, configuration) {
...
function unRegisterTooltip(tooltip, tooltipIconButtonClass) {
    ...
    */
    ...
    if (focusableElements.length > 0) {
        ...
        if (childHandlers) {
            ...
            if (positionHandler)
                focusableChild.removeEventListener('focus', positionHandler);
            if (keyHandler)
                focusableChild.removeEventListener('keydown', keyHandler);
    ...
    if (closeSpanButton) {
        ...
        if (closeHandler)
            closeSpanButton.removeEventListener("click", closeHandler);
    }
}
/*
